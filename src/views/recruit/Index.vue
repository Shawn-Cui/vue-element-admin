  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :data="newsList" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" :label="$t('table.job')">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.job || jobs[scope.$index % 4]}}</span>
          <!-- <el-tag>{{scope.row.type | typeFilter}}</el-tag> -->
        </template>
      </el-table-column>
      <el-table-column width="150px" align="center" :label="$t('table.recruitNumber')">
        <template slot-scope="scope">
          <span>{{scope.row.author.length + 5}}</span>
        </template>
      </el-table-column>
    ...
    handleDelete(index, row) {
      this.$confirm('此操作将永久删除该招聘，是否继续？', '提示', {
        ...
        for (let i = 0; i < this.newsList.length; i++) {
          if (this.newsList[i].id === row.id) {
            delete this.newsList[i]
          }
        }
